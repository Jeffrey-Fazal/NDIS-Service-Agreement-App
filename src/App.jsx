...
// --- Constants ---
const NDIS_RATES_URL = '/ndisrates2025.json';
const SERVICE_AGREEMENT_URL = '/sa-sjon-generic.json';
const HEADER_IMAGE_URL = '/images/header.png';
const FOOTER_IMAGE_URL = '/images/footer.png';
...
const FloatingNav = ({ totalPay, totalHours, transportCost }) => {
    const navItems = [
        { href: '#instructions-section', label: '0. Instructions' },
...
const InstructionsSection = () => {
    ...
    return (
        <Section id="instructions-section" title="0. Instructions" color="gray">
            <button
                onClick={() => setIsOpen(!isOpen)}
                className="w-full text-left font-semibold text-gray-700 flex justify-between items-center mb-2 p-2 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-400"
            >
                <span>Click here to {isOpen ? 'hide' : 'show'} instructions</span>
                <span className="transform transition-transform duration-200" style={{ transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}>▼</span>
            </button>
            {isOpen && (
                <div className="text-left space-y-2 text-gray-600 animate-fade-in mt-4 border-t pt-4">
                    <p>This tool helps you create quotes and service agreements. Follow these steps:</p>
                    <ol className="list-decimal list-inside space-y-1">
                        <li>Note the tool is set for regular QLD pricing and holidays and only works with hourly supports</li>
                        <li>Start by setting the <strong>Date Range</strong> and how often the service recurs (e.g., Weekly).</li>
                        <li>(Optional) If you have a fixed budget, use the <strong>Budget Information</strong> section to get suggestions for hours and travel.</li>
                        <li>Select the days of the week services occur and enter the hours for each in the <strong>Applicable Days & Hours</strong> section.</li>
                        <li>Use the <strong>NDIS Rate Finder</strong> to automatically look up and populate service rates. You can override these in the <strong>Manual Rates</strong> section.</li>
                        <li>Add any travel costs using the <strong>Transport Calculator</strong>.</li> (You'll need to look up the transport line items manually)
                        <li>(Optional) You can now change the hours in section 3 (3. Applicable Days & Hours per Day) to get suggsted hours based on the budget</li>
                        <li>Once your totals look correct, go to <strong>Save Calculation as Quote</strong>, give it a description, and save it. You can save multiple quotes.</li>
                        <li>Always check quotes against the <a href="https://www.ndis.gov.au/providers/pricing-arrangements">NDIS Pricing Arrangements and Price Limits</a>  &/or a tool like <a href="https://planpartners.com.au/tools/ndis-price-guide">plan partners price guide</a></li>
                        <li>Finally, fill in the participant's details in the <strong>Generate Service Agreement</strong> section and click either "Generate & Print" or "Download as Word".</li>
                    </ol>
                </div>
            )}
        </Section>
...
function App() {
    ...
    const getAgreementHtml = () => {
        ...
        const dataMap = {
            ...
            '{{provider.abn}}': PROVIDER_DETAILS.abn,
            '{{provider.email}}': PROVIDER_DETAILS.email,
            '{{provider.phone}}': PROVIDER_DETAILS.phone,
            '{{provider.address}}': PROVIDER_DETAILS.address,
            '{{provider.website}}': PROVIDER_DETAILS.website,
        };
    ...
    const handleGenerateWordDoc = async () => {
        ...
        try {
            // Only fetch the template - no longer need to fetch images
            const templateResponse = await fetch('/template.docx');
            
            if (!templateResponse.ok) {
                throw new Error(`Failed to fetch template. Status: ${templateResponse.status}`);
            }
            
            const templateContent = await templateResponse.arrayBuffer();
            
            // Create PizZip instance with template content
            const zip = new PizZip(templateContent);
            
            // No longer need the image module since images are hardcoded
            const nullGetter = () => "";
            
            // Create Docxtemplater without image module
            ...
            // Prepare data
            const isPlanManaged = serviceAgreementInfo.fundingType.includes("Plan Managed");

            const data = {
                participantName: serviceAgreementInfo.participantName,
                participantAddress: serviceAgreementInfo.participantAddress,
                ndisNumber: serviceAgreementInfo.ndisNumber,
                representativeName: serviceAgreementInfo.representativeName,
                agreementDate: serviceAgreementInfo.agreementDate,
                self_funded_check: serviceAgreementInfo.fundingType.includes("Self Funded") ? '☒' : '☐',
                plan_managed_check: serviceAgreementInfo.fundingType.includes("Plan Managed") ? '☒' : '☐',
                ndia_managed_check: serviceAgreementInfo.fundingType.includes("NDIA Managed") ? '☒' : '☐',
                is_plan_managed: isPlanManaged,
                planManager: isPlanManaged ? serviceAgreementInfo.planManager : null,
                pm_name: serviceAgreementInfo.planManager.name,
                pm_address: serviceAgreementInfo.planManager.address,
                pm_phone: serviceAgreementInfo.planManager.phone,
                pm_email: serviceAgreementInfo.planManager.email,
                provider_name: PROVIDER_DETAILS.name,
                provider_abn: PROVIDER_DETAILS.abn,
                provider_address: PROVIDER_DETAILS.address,
                provider_phone: PROVIDER_DETAILS.phone,
                provider_email: PROVIDER_DETAILS.email,
                provider_website: PROVIDER_DETAILS.website,
                quotes: savedQuotes.map(quote => {
                    const items = [];
                    const serviceDetails = [
                        { label: 'Weekday Day', data: quote.results.weekdayHours, pay: quote.results.weekdayPay, rate: quote.rates.weekday },
                        { label: 'Weekday Evening', data: quote.results.weekdayEveningHours, pay: quote.results.weekdayEveningPay, rate: quote.rates.weekdayEvening },
                        { label: 'Saturday', data: quote.results.saturdayHours, pay: quote.results.saturdayPay, rate: quote.rates.saturday },
                        { label: 'Sunday', data: quote.results.sundayHours, pay: quote.results.sundayPay, rate: quote.rates.sunday },
                        { label: 'Public Holiday', data: quote.results.publicHolidayHours, pay: quote.results.publicHolidayPay, rate: quote.rates.publicHoliday },
                    ];
                    
                    serviceDetails.forEach(d => {
                        if (d.data > 0) items.push({
                            label: d.label,
                            name_and_number: `${d.rate.name || 'N/A'}\n${d.rate.number || 'Manual Rate'}`,
                            rate: `$${formatNumber(parseFloat(d.rate.rate))}`,
                            quantity: formatNumber(d.data),
                            total: `$${formatNumber(d.pay)}`
                        });
                    });
                    
                    if (quote.transport?.npt?.cost > 0) {
                        items.push({
                            label: 'Transport (NPT)',
                            name_and_number: `${quote.transport.npt.name || ''}\n${quote.transport.npt.itemCode || ''}`,
                            rate: `$${formatNumber(parseFloat(quote.transport.npt.rate))} / km`,
                            quantity: `${quote.transport.npt.totalKm} km`,
                            total: `$${formatNumber(quote.transport.npt.cost)}`
                        });
                    }
                    
                    if (quote.transport?.abt?.cost > 0) {
                        items.push({
                            label: 'Transport (ABT)',
                            name_and_number: `${quote.transport.abt.name || ''}\n${quote.transport.abt.itemCode || ''}`,
                            rate: `$${formatNumber(parseFloat(quote.transport.abt.rate))} / km`,
                            quantity: `${quote.transport.abt.totalKm} km`,
                            total: `$${formatNumber(quote.transport.abt.cost)}`
                        });
                    }
                    ...
                    return {
                        ...
                        items: items,
                        total_hours: formatNumber(quote.totalHours),
                        total_pay: formatNumber(quote.totalPay),
                    };
                }),
            };
            ...
            saveAs(out, `Service_Agreement_${serviceAgreementInfo.participantName || 'Generated'}.docx`);
        } catch (error) {
            console.error("Error generating Word document:", error);
            if (error.properties && error.properties.errors) {
                const templateErrors = error.properties.errors.map(e => `[${e.id}] ${e.properties.explanation}`).join('\n');
                setError(`Template Error:\n${templateErrors}`);
            } else {
                setError(`Could not generate Word document. Error: ${error.message}`);
            }
        }
    };
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 sm:p-6 lg:p-8 font-sans antialiased text-gray-800">
            ...
            <main className="max-w-4xl w-full mx-auto bg-white shadow-xl rounded-xl p-6 sm:p-8 space-y-6">
                <div className="text-center no-print">
                    <h1 className="text-3xl sm:text-4xl font-extrabold text-indigo-700">Service Agreement Generator</h1>
                    <p className="mt-2 text-gray-600">An internal tool for quoting and generating service agreements.</p>
                </div>
            ...
            <footer className="text-center py-4 text-sm text-gray-500 no-print">
                <span>Created with ❤️ by Jeffrey Fazal</span>
                <a
                    href="https://github.com/Jeffrey-Fazal/NDIS-Service-Agreement-App"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center ml-3 text-gray-500 hover:text-gray-800"
